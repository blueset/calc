...
function evaluate(input: string): Value {
  ...
  return (
    results.get(firstLine) || {
      kind: "error",
      error: { type: "RuntimeError", message: "No result" },
    }
  );
}
...
describe("Evaluator", () => {
  describe("Literal Evaluation", () => {
    it("should evaluate number literals", () => {
      const result = evaluate("42") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(42);
      expect(getUnit(result)).toBeUndefined();
    });

    it("should evaluate decimal numbers", () => {
      const result = evaluate("3.14159") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBeCloseTo(3.14159);
    });

    it("should evaluate scientific notation", () => {
      const result = evaluate("2e3") as NumericValue;
      expect(result.kind).toBe("value");
    ...
    it("should evaluate mathematical constants", () => {
      const piVal = evaluate("pi") as NumericValue;
      expect(piVal.kind).toBe("value");
      expect(piVal.value).toBeCloseTo(Math.PI);

      const eVal = evaluate("e") as NumericValue;
      expect(eVal.kind).toBe("value");
      expect(eVal.value).toBeCloseTo(Math.E);
    });

    it("should evaluate numbers with units", () => {
      const result = evaluate("5 m") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(5);
      expect(getUnit(result)).toBeDefined();
      expect(getUnit(result)?.dimension).toBe("length");
    });
  });

  describe("Binary Arithmetic Operations", () => {
    it("should add two dimensionless numbers", () => {
      const result = evaluate("2 + 3") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(5);
    });

    it("should subtract dimensionless numbers", () => {
      const result = evaluate("10 - 3") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(7);
    });

    it("should multiply dimensionless numbers", () => {
      const result = evaluate("4 * 5") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(20);
    });

    it("should divide dimensionless numbers", () => {
      const result = evaluate("10 / 2") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(5);
    });
    ...
    it("should add numbers with same unit", () => {
      const result = evaluate("5 m + 3 m") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(8);
      expect(getUnit(result)?.id).toBe("meter");
    });

    it("should add numbers with compatible units (different scale)", () => {
      const result = evaluate("5 m + 200 cm") as NumericValue;
      expect(result.kind).toBe("value");
  ...
  describe("Bitwise Operations", () => {
    it("should perform bitwise AND", () => {
      const result = evaluate("12 & 10") as NumericValue;
      expect(result.kind).toBe("value");
  ...
  describe("Unary Operations", () => {
    it("should negate numbers", () => {
      const result = evaluate("-5") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(-5);
    });

    it("should negate numbers with units", () => {
      const result = evaluate("-(3 m)") as NumericValue;
      expect(result.kind).toBe("value");
  ...
  describe("Postfix Operations", () => {
    ...
    it("should calculate factorial of 0", () => {
      const result = evaluate("0!") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(1);
    });
  ...
  describe("Function Calls", () => {
    it("should evaluate sqrt", () => {
      const result = evaluate("sqrt(16)") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(4);
    });

    it("should evaluate sin (radian mode)", () => {
      const result = evaluate("sin(0)") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBeCloseTo(0);
    });

    it("should evaluate abs", () => {
      const result = evaluate("abs(-5)") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(5);
    });

    it("should evaluate floor", () => {
      const result = evaluate("floor(3.7)") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(3);
    });

    it("should evaluate ceil", () => {
      const result = evaluate("ceil(3.2)") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(4);
    });

    it("should evaluate round", () => {
      const result = evaluate("round(3.7)") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(4);
    });

    it("should evaluate log", () => {
      const result = evaluate("log10(100)") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBeCloseTo(2); // log10(100) = 2
    });

    it("should evaluate ln", () => {
      const result = evaluate("ln(e)") as NumericValue;
      expect(result.kind).toBe("value");
  ...
  describe("Unit Conversions", () => {
    it("should convert between units of same dimension", () => {
      const result = evaluate("5 km to m") as NumericValue;
      expect(result.kind).toBe("value");
    ...
    it("should convert meters to centimeters", () => {
      const result = evaluate("2.5 m to cm") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(250);
      expect(getUnit(result)?.id).toBe("centimeter");
    });

    it("should convert feet to inches", () => {
      const result = evaluate("2 ft to in") as NumericValue;
      expect(result.kind).toBe("value");
    ...
    it("should handle chained conversions", () => {
      const result = evaluate("5 km to m to cm") as NumericValue;
      expect(result.kind).toBe("value");
  ...
  describe("Conditional Expressions", () => {
    it("should evaluate true branch", () => {
      const result = evaluate("if true then 10 else 20") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(10);
    });

    it("should evaluate false branch", () => {
      const result = evaluate("if false then 10 else 20") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(20);
    });

    it("should evaluate conditional with comparison", () => {
      const result = evaluate("if 5 > 3 then 100 else 200") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(100);
    });

    it("should convert numbers to boolean in condition", () => {
      const result = evaluate("if 0 then 10 else 20") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(20); // 0 is falsy
    });

    it("should treat non-zero as true", () => {
      const result = evaluate("if 5 then 10 else 20") as NumericValue;
      expect(result.kind).toBe("value");
  ...
  describe("Variable Assignments", () => {
    it("should assign and retrieve variables", () => {
      ...
      const result = results.get(secondLine) as NumericValue;
  ...
  describe("Grouped Expressions", () => {
    it("should respect parentheses for precedence", () => {
      const result = evaluate("(2 + 3) * 4") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(20);
    });

    it("should evaluate nested parentheses", () => {
      const result = evaluate("((2 + 3) * 4) + 1") as NumericValue;
      expect(result.kind).toBe("value");
  ...
  describe("Operator Precedence", () => {
    ...
    it("should handle power before multiplication", () => {
      const result = evaluate("2 * 3 ^ 2") as NumericValue;
      expect(result.kind).toBe("value");
      expect(result.value).toBe(18); // 2 * (3 ^ 2)
    });

    it("should handle unary minus with precedence", () => {
      const result = evaluate("-2 ^ 2") as NumericValue;
      expect(result.kind).toBe("value");
