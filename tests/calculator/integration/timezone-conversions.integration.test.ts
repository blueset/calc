...
1970 Jan 01 14:00 UTC+2 to UTC-1`);
    ...
    it("should convert to named timezones", () => {
      const result =
        calculator.calculate(`1970 Jan 01 14:00 UTC to America/New_York
...
1970 Jan 01 14:00 UTC to Sydney`);
    ...
    it("should support IANA timezone database names", () => {
      const result =
        calculator.calculate(`1970 Jan 01 14:00 UTC to America/Los_Angeles
...
1970 Jan 02 03:00 UTC to UTC-8`);
  ...
  describe("Daylight Saving Time Handling", () => {
    it("should handle DST transitions in America/New_York", () => {
      // Winter (EST: UTC-5)
      const winterResult = calculator.calculate(
        `1970 Jan 15 12:00 UTC to America/New_York`,
      );
      expect(winterResult.results[0].result).toMatch(/UTC-5$/);

      // Summer (EDT: UTC-4)
      const summerResult = calculator.calculate(
        `1970 Jul 15 12:00 UTC to America/New_York`,
      );
    ...
    it("should handle DST transitions in Europe/London", () => {
      // Winter (GMT: UTC+0)
      const winterResult = calculator.calculate(
        `2000 Jan 15 12:00 UTC to Europe/London`,
      );
      expect(winterResult.results[0].result).toBe("2000-01-15 Sat 12:00 UTC");

      // Summer (BST: UTC+1)
      const summerResult = calculator.calculate(
        `2000 Jul 15 12:00 UTC to Europe/London`,
      );
...
1970 Jan 01 14:00 UTC+5 to UTC+5`);
    ...
    it("should handle conversion chains", () => {
      const result = calculator.calculate(
        `1970 Jan 01 00:00 UTC to America/New_York to Tokyo`,
      );
