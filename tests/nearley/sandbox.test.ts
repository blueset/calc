...
function toString(node: any, indent: number = 0): string {
  ...
  if (Array.isArray(node)) {
    ...
    const items = node.map((n) => `${childSpaces}${toString(n, indent + 1)}`).join(",\n");
  ...
  if (node && typeof node.type === "string") {
    ...
    if (type === "UnitWithExponent") {
      return node.unit.matched === 'identifier' ? `«${node.unit.name}»:${node.exponent}` : `"${node.unit.name}":${node.exponent}`;
    } else if (type === "Units") {
      const numerators = node.numerators?.map((n: any) => toString(n, 0)).join(" ");
      const denominators = node.denominators?.map((d: any) => toString(d, 0)).join(" ");
      if (numerators?.length && denominators?.length) {
        return `[${numerators} / ${denominators}]`;
      } else if (numerators?.length) {
        return `[${numerators}]`;
    ...
    const params = entries
      .map(([key, value]) => `${childSpaces}${key}=${toString(value, indent + 1)}`)
      .join(",\n");
