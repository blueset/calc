...
function AstNode({ node, depth = 0 }: { node: any; depth?: number }) {
  ...
  if (typeof node !== 'object') {
    return <span className="text-green-600 dark:text-green-400">{JSON.stringify(node)}</span>
  }
  ...
  const entries = Object.entries(node).filter(([k]) => k !== 'type' && k !== 'location')
  ...
  return (
    <div>
      ...
      >
        ...
        {type && <Badge variant="outline" className="px-1 py-0 text-xs">{type}</Badge>}
        ...
        <div className="ml-3 pl-2 border-border border-l">
          {entries.map(([key, value]) => (
            <div key={key} className="flex items-start gap-1">
              <span className="text-blue-600 dark:text-blue-400 text-xs shrink-0">{key}:</span>
...
export function DebugPanel({ ast, errors }: DebugPanelProps) {
  const totalErrors = errors.lexer.length + errors.parser.length + errors.runtime.length

  return (
    <div className="flex flex-col border-border border-t h-full">
      ...
      <ScrollArea className="flex-1 overflow-auto">
        <div className="p-3 font-mono text-xs">
          ...
          {totalErrors > 0 && (
            <div className="mb-3">
              <h4 className="mb-1 font-semibold text-destructive">Errors</h4>
              {errors.parser.map((e, i) => (
                <div key={`p${i}`} className="text-destructive whitespace-pre-wrap">
                  Line {e.line}: {e.error.message}
                </div>
              ))}
              {errors.runtime.map((e, i) => (
                <div key={`r${i}`} className="text-destructive whitespace-pre-wrap">
                  {e.location ? `Line ${e.location.line}:` : ''} {e.message}
                </div>
              ))}
            </div>
          )}
          ...
          {ast ? <AstNode node={ast} /> : <span className="text-muted-foreground">No AST</span>}
        </div>
      </ScrollArea>
    </div>
  )
