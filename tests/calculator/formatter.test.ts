...
import type { Unit } from "../../src/calculator/types/types";
...
describe("Formatter", () => {
  describe("Number Formatting", () => {
    ...
    it("should format numbers with decimal comma", () => {
      ...
      const formatter = new Formatter(settings);
      const value: NumericValue = { kind: "value", terms: [], value: 123.45 };
    ...
    it("should format negative numbers correctly", () => {
      ...
      const formatter = new Formatter(settings);
      const value: NumericValue = { kind: "value", terms: [], value: -1234.56 };
    ...
    it("should format special values (Infinity, NaN)", () => {
      const formatter = new Formatter();
      expect(
        formatter.format({ kind: "value", value: Infinity, terms: [] }),
      ).toBe("Infinity");
      expect(
        formatter.format({ kind: "value", value: -Infinity, terms: [] }),
      ).toBe("-Infinity");
      expect(formatter.format({ kind: "value", value: NaN, terms: [] })).toBe(
        "NaN",
      );
    });
    ...
    it("should format zero correctly", () => {
      const formatter = new Formatter();
      const value: NumericValue = { kind: "value", terms: [], value: 0 };
    ...
    it("should format numbers with East Asian grouping (4 digits)", () => {
      ...
      const formatter = new Formatter(settings);
      const value: NumericValue = { kind: "value", terms: [], value: 12345678 };
  ...
  describe("Presentation Format", () => {
    ...
    it("should format scientific notation", () => {
      ...
      expect(formatter.formatPresentation(0.00123, "scientific")).toBe(
        "1.23e-3",
      );
    });
  ...
  describe("Auto Precision", () => {
    ...
    it("should use auto precision for small numbers", () => {
      const settings = createSettings({ precision: -1 });
      const formatter = new Formatter(settings);
      const value: NumericValue = { kind: "value", terms: [], value: 0.00001 };
    ...
    it("should use auto precision for normal numbers", () => {
      const settings = createSettings({ precision: -1 });
      const formatter = new Formatter(settings);
      const value: NumericValue = { kind: "value", terms: [], value: 123.456 };
