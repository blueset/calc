...
1970 Jan 01 01:00 UTC-330 to RFC 9557`);
      ...
      expect(result.results[1].result).toBe(
        "1970-01-01T01:00:00+05:00[+05:00]",
      );
      expect(result.results[2].result).toBe(
        "1970-01-01T01:00:00-03:30[-03:30]",
      );
    });
...
1970 Jan 01 00:00 to Unix seconds`);
    ...
    it("should convert to Unix milliseconds", () => {
      const result =
        calculator.calculate(`1970 Jan 01 00:00 UTC to Unix milliseconds
...
dt to Unix milliseconds`);
    ...
    it("should apply conversions in sequence", () => {
      const result = calculator.calculate(
        `1970 Jan 01 01:00 UTC to Unix to ISO 8601`,
      );
