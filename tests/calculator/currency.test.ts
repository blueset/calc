...
import {
  CurrencyConverter,
  type CurrencyValue,
} from "../../src/calculator/currency";
import type { ExchangeRatesDatabase } from "../../src/calculator/types/types";
...
describe("CurrencyConverter", () => {
  ...
  describe("Conversion Through Base Currency", () => {
    it("should support all 6 conversion combinations with single base currency", () => {
      ...
      const usdToGbp = converter.convert(
        { amount: 100, currencyCode: "USD" },
        "GBP",
      );
      ...
      const eurToUsd = converter.convert(
        { amount: 85, currencyCode: "EUR" },
        "USD",
      );
      expect(eurToUsd.amount).toBeCloseTo(100.0, 1);

      // 4. EUR → GBP (through base: (1/0.85) * 0.73)
      const eurToGbp = converter.convert(
        { amount: 85, currencyCode: "EUR" },
        "GBP",
      );
      ...
      const gbpToUsd = converter.convert(
        { amount: 73, currencyCode: "GBP" },
        "USD",
      );
      expect(gbpToUsd.amount).toBeCloseTo(100.0, 1);

      // 6. GBP → EUR (through base: (1/0.73) * 0.85)
      const gbpToEur = converter.convert(
        { amount: 73, currencyCode: "GBP" },
        "EUR",
      );
...
describe("Currency Formatting Tests", () => {
  ...
  describe("Currency-Specific Decimal Places", () => {
    ...
    it("should display ambiguous symbol not dimension", () => {
      ...
      expect(result.results[0].result).toBe("$150.00");
