...
2020 Dec 31 + 1 year`);
    ...
    it("should handle mixed time unit arithmetic", () => {
      const result =
        calculator.calculate(`2023 Jan 1 12:00 + 1 day 2 hours 30 minutes
...
01:00 - 2 hours`);
        expect(result.results[0].result).toMatch(
          /\d{4}-\d{2}-\d{2} \w{3} 00:30/,
        );
        expect(result.results[1].result).toMatch(
          /\d{4}-\d{2}-\d{2} \w{3} 23:00/,
        );
      });
    });
...
2023 Jan 15 14:30 UTC + 1 day`);
      ...
      it("should handle DST transitions", () => {
        ...
        const result = calculator.calculate(
          "2023 Mar 12 01:00 America/New_York + 2 hours",
        );
